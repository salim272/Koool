...
export const mapRawFeatureBrandsToFeatureBrands = (rawUsers: RawFeatureBrand[]): FeaturedBrand[] => {
    return rawUsers
        .filter((item: RawFeatureBrand) =>
            (item.status?.toUpperCase().includes('FEATURED')) && item.brandImgFileName != null)
        .map(mapFeaturedBrand)
        .sort((a: RawFeatureBrand, b: RawFeatureBrand) => a.name.localeCompare(b.name));
}
